...
class Settings(BaseSettings):
    ...
    # Окно скользящих медианы/MAD для z-score (с): «норма» считается локально по
    # записи, а не один раз на весь канал — устойчивость к нестационарности
    # (дрейф уровня/амплитуды давал длинные зоны zscore_outlier, N9/шаг 2.3).
    # Запись короче окна считает по глобальной оценке (фолбэк).
    zscore_window_sec: float = Field(default=30.0)
