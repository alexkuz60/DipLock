...
class Settings(BaseSettings):
    ...
    # Атлас структур (aparc+aseg): если точка попала в неразмеченный воксель,
    # подпись структуры берётся у ближайшей размеченной ячейки в этом радиусе
    # (мм); дальше — честное «не определено». Сферическая сетка быстрого расчёта
    # ставит узлы между вокселями атласа и за край мозга, поэтому без радиуса
    # ~треть точек оставалась без структуры (находка ручной проверки 19.09.2026).
    atlas_structure_snap_mm: float = Field(default=5.0)
