...
`state/dipoleCalc.test.ts`: отпечаток параметров и сверка с результатом). Всего **364 теста Vitest
(38 файлов) и 150 pytest** (из них 26 — геометрия, укладка и кэш среза МРТ, 14 — спектр и быстрый
расчёт). `mriProjections.test.ts` дополнительно фиксирует знаки осей MNI, подписи краёв и единый
масштаб проекций с пропорциями колонок раздела.
...
- Полоса фильтра расчёта пока фиксирована (1–40 Гц) — форма фильтров δ/θ/α/β/γ это отдельный срез
  (`docs/ui.md` §3.3). Живёт она в `dipoleCalc`, а не в `edfParams`, чтобы расчёт не менялся
  «незаметно» от правок предподготовки записи.
