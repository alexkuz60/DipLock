...
- События EDF+ (шаг 2.7, MNE 1.13): TAL-разделитель длительности — `\x15`, а не `\x14`
  (иначе «длительность» читается отдельным описанием); `mne.find_events(raw, stim_channel='auto')`
  падает ('auto' — не имя канала: нужен вызов без параметра или конкретное имя);
  `raw.drop_channels` не принимает `verbose`; `events_from_annotations` без совпадений кидает
  свой `ValueError` — проверка описания и свои тексты ошибок собираются до вызова
  (`docs/rules/events.md`).
