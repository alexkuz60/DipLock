...
2. **Задача = job, и только по кнопке.** Расчёт стартует исключительно `POST …/{kind}`:
   `202` + `job_id` → поллинг `GET …/{job_id}` → `result_url` из `_job_status`.
   Новый вид задачи (`kind`) обязан быть добавлен и в `_job_status` (сборка `result_url`),
   и в `_drop_signal_cache` (чистка кэшей записи — дисковых и RAM-кэша подготовленного сигнала)
   — иначе результат «потеряется» после вытеснения.
...
4. **ETag/304 и версии ассетов** — через общий помощник (`_asset_response` в `routes.py`),
   а не вручную: сейчас ручные реализации ещё есть в нескольких местах (см. `audit-2026-09.md`,
   находка о 6 копиях). Клиент кэширует по URL, поэтому URL картинки/сетки обязан нести параметры
   расчёта и `?v={asset_version}`.
