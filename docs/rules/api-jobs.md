...
11. **Эпохи — часть контракта legacy-результата (F21, этап 7).** `AnalyzeResponse` отдаёт поле
   ...
   `has_artifact`, `band_powers`), а не только прошедшие reject: это единственный источник строк
   таблицы `epochs` в БД и связи `dipoles.epoch_id`. `has_artifact=true` означает «эпоха не вошла в
   анализ» (reject по амплитуде или неполное окно в конце записи), `band_powers` у таких эпох пуст
