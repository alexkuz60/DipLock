...
нарезанные эпохи, а `n_epochs_dropped` — сколько отброшено (BAD_-зоны детекторов или хвост записи; раньше оба числа совпадали).
...
пересчитать»). Примитивы: `Placeholder`, `LoadingBlock`, `ErrorBlock`, `InfoRow`, `ErrorBoundary`
(страховка рендера: падение компонента даёт блок ошибки с перезагрузкой, а не белый экран).
...
панель опций имеет `aria-label`, заголовки-аккордеоны её секций — `aria-expanded` и `aria-controls`; статусы проверок сервера подписаны текстом, а не только цветом;
