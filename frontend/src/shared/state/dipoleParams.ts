 ...
 *
 * Кратность узла сетки (`withOverlapCounts`) — не параметр: она всегда считается по
 * видимому слою, потому что кольцо позиции читает её размером и заливкой (поправка
 * ручной проверки, 18.09.2026). Отдельного переключателя нет: это свойство данных, а
 * не предпочтение просмотра.
 */
