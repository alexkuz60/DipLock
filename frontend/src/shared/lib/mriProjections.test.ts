 ...
 * ориентациям, следы соседних срезов. Фикстуры слоёв (силуэт головы, схема среза,
 * поля Бродмана) вынесены в `mriDemoShapes.test.ts` — здесь только геометрия.
 */
...
describe('геометрия проекций мозга', () => {
  ...
  it('центр фигуры — начало координат плоскости, pxToNormalized обратна normalizedToPx', () => {
    for (const plane of PROJECTION_PLANES) {
      ...
      // Однокоординатные помощники (сетка, следы срезов) — тот же перевод по оси
      expect(xOfNormalized(plane, 0)).toBe(center.x)
      expect(yOfNormalized(plane, 0)).toBe(center.y)
      expect(xOfNormalized(plane, -1)).toBe(corner.x)
      expect(yOfNormalized(plane, 1)).toBe(corner.y)
    }
  })
