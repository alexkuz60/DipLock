/**
 * Три ортогональные проекции мозга (срез 3.1): MNI-геометрия, сетка, следы срезов,
 * подписи краёв и чистая арифметика наведения.
 *
 * Модуль описывает **геометрию**, а не данные: она не меняется, когда на смену
 * фикстурам придут реальные срезы, меш и поля. Поэтому здесь только то, что
 * переживёт этот переход. Условные фигуры (силуэт головы, схема среза, поля
 * Бродмана) вынесены в `shared/lib/mriDemoShapes.ts` — файл-заглушка, который
 * удаляется целиком, как только срез `mri`, контуры атласа и поля покроют панель.
 *
 * Точки диполей приходят в MNI-координатах (мм), поэтому и вся арифметика ведётся
 * в MNI: система координат одна — иначе срезы, поля Бродмана и диполи не совпали бы.
 *
 * Реальные MNI-срезы (том fsaverage `mri/T1.mgz` → `mni305.cor.mgz`, MNI152), меш
 * и Brodmann-поля — серверный кэшируемый актив (`docs/ui.md` §3.3, §12).
 *
 ...
 * растягивается. Всё, что живёт в нормализованных координатах (сетка, следы
 * срезов, фигуры `mriDemoShapes`), получает этот масштаб через `normalizedToPx`.
 *
...
/**
 * Пиксельная горизонталь по нормализованной координате u (сетка, следы срезов).
 *
 * Отдельные однокоординатные функции, а не общий перевод точки: сетке нужна
 * только одна ось, и вторая координата (0) в расчёт не попадает — вызывающий код
 * не «изобретает» нулевую точку.
 */
export function xOfNormalized(plane: ProjectionPlane, u: number): number {
  return normalizedToPx({ u, v: 0 }, plane).x
}

/** Пиксельная вертикаль по нормализованной координате v (ось v растёт вверх). */
export function yOfNormalized(plane: ProjectionPlane, v: number): number {
...
/**
 * Эллипс фигуры (структура среза, поле Бродмана) в пикселях фигуры.
 *
 ...
 * поле считается по тем же эллипсам, что нарисованы (`brodmannAreaAt` в
 * `mriDemoShapes.ts`), и «второго, невидимого» слоя для мыши в разделе нет.
 * Пересчитывать эту арифметику в компоненте нельзя — разъехавшиеся эллипсы сделают
 * подсветку поля ложной, а поймать это глазами почти невозможно.
 */
