 ...
 *
 * Это фикстура разработки: вьюер рисует её по кнопке «Демо-сигнал». Реальные
 * сигналы записи приходят тем же путём через кадр `SignalFrame`
 * (срез 2.5, `shared/lib/signalFrame.ts`) — вьюер не знает, откуда данные.
 * Отсюда берётся только набор каналов, sfreq, длительность и отсчёты в мкВ.
 */
